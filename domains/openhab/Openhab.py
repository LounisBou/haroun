...
class Openhab(Domain):
    ...
    def question(self, item_type, room = None, value = None, question_trigger = None, orphan = None):
        ...
        # Get item type lang.
        item_type_lang = self.get_slot(item_type)
        
        # Check if room not provide.
        if not room :
            # Ask for room information.
            return self.say(
                "openhab.question.what_room",
                item_type = item_type_lang
            )
            
        # Get room lang.
        room_lang = self.get_slot(room)
        ...
        if item : 
            ...
            # If item state is a slot, retrieve it.
            if self.get_slot(item_state) :
                item_state = self.get_slot(item_state)
        else: 
            # [DEBUG] Say item not found.
            # Ask for room information.
            return self.say(
                "openhab.question.item_not_found",
    ...
    def action(self, item_type, room = None, value = None, action_trigger = None, orphan = None):
        ...
        if not room :
            # Ask for room information.
            return self.say("openhab.action.what_room", item_type = item_type_lang)
